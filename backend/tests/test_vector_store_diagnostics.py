    ...
    def test_existing_data_availability(self):
        ...
        try:
            ...
            if course_count > 0:
                ...
                results = vector_store.search("Python")
                print(f"Search results for 'Python': {len(results.documents)} documents")
    ...
    def test_search_real_course_content(self):
        ...
        try:
            ...
            for query in test_queries:
                ...
                print(f"Query '{query}': {len(results.documents)} docs, error: {results.error}")
            
            # At least one query should return results if data exists
            total_results = sum(r["doc_count"] for r in search_results.values())
            
            if total_results == 0:
                print("WARNING: No search results for any test query - possible data issue!")
                
                # Check if there's any content at all
                content_count = vector_store.course_content.count()
                if content_count == 0:
                    pytest.fail("No course content found in vector store - this is likely the cause of 'query failed' errors")
                else:
                    print(f"Content exists ({content_count} chunks) but no search results - possible embedding/search issue")
        ...
        try:
            ...
            if catalog_count > 0:
                ...
                for course in all_courses[:3]:  # Show first 3
                    print(f"  - {course.get('title', 'Unknown')}: {len(course.get('lessons', []))} lessons")
            ...
            if content_count > 0:
                ...
                for i, doc in enumerate(content_sample.get('documents', [])[:3]):
                    ...
                    print(f"  - {doc[:100]}... (Course: {meta.get('course_title', 'Unknown')})")
