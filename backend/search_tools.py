...
class CourseOutlineTool(Tool):
    ...
    def execute(self, course_name: str) -> str:
        ...
        Args:
            course_name: Course name or partial name
            
        Returns:
            Formatted course outline with title, link, and lessons
        """
        
        # First resolve the course name using the same logic as search
    ...
    def _format_outline(self, course_metadata: Dict[str, Any]) -> str:
        ...
        link = course_metadata.get('course_link', '')
        lessons = course_metadata.get('lessons', [])
        
        outline = [f"**{title}**"]
        
        # Only show link if it exists and is not empty
        if link and link.strip():
            outline.append(f"🔗 [View Course]({link})")
